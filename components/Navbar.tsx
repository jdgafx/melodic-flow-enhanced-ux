          ...
          <Link to="/" className="flex items-center gap-3 cursor-pointer group select-none">
            ...
            <div className="relative w-11 h-11 rounded-xl bg-gradient-to-b from-blue-500 to-purple-600 p-[1px] shadow-lg shadow-purple-500/20 group-hover:shadow-purple-500/40 transition-all duration-300">
               <div className="w-full h-full bg-[#0a0a0f] rounded-[10px] flex items-center justify-center relative overflow-hidden">
                  ...
                  <Zap className="w-5 h-5 text-transparent fill-current" style={{ fill: 'url(#logo-gradient-nav)', filter: 'drop-shadow(0 0 8px rgba(168, 85, 247, 0.6))' }} />
          ...
          <div className="hidden md:flex items-center bg-white/5 rounded-full px-2 py-1 border border-white/5 backdrop-blur-sm">
            {navItems.map((item) => (
              <Link 
                key={item.label} 
                to={item.href}
                className={`px-5 py-2 text-sm font-medium transition-all rounded-full ${location.pathname === item.href ? 'text-white bg-white/10 shadow-[0_0_10px_rgba(255,255,255,0.1)]' : 'text-gray-300 hover:text-white hover:bg-white/5'}`}
              >
                {item.label}
              </Link>
            ))}
